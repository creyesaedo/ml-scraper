...
- Decodo-side `400` ("Something went wrong. Please try again later") bursts **and** `5xx` gateway errors retry up to `DECODO_TRANSIENT_MAX_RETRIES` times (default **10**). The two differ on billing per [Decodo's response-codes table](https://help.decodo.com/docs/web-scraping-api-response-codes): `5xx`/`524`/`613` are **never billed** (so retrying is free), but `400` is a `4xx` and **bills when the body is non-empty** — so a sustained 400 burst costs one request per attempt. We still retry the full budget (the alternative is 0 products), but it is not free. Both were observed lasting many seconds in the field (especially MCO); without a generous budget a *sustained* 400 burst would churn whole categories to 0 products.
- HTTP `429` (rate cap — **billed when the body is non-empty**, a `4xx`) keeps a **single** retry — hammering a rate cap won't clear it; the sliding-window limiter is the real guard.
- Network errors (DNS/TCP/TLS) keep a **single** retry — a second failure indicates a real outage.

Account errors (`401`/`402`/`403`) are permanent and never retried. Note: Decodo's response-codes table only documents `401` (bad/expired token) and `403` (no access) as account-level codes — both bill when the body is non-empty. `402` ("out of balance") is **not** in that table; it is matched defensively in `isDecodoAccountError` but is unverified against the docs.
...
**Billing-aware error handling:** by Decodo's response-code rules, `200`/`204` always bill, `4xx` with non-empty body bills, but `500`/`524`/`613` ("failed to scrape") do not. Two distinct retries live in `scrapeWithWait`:
- **Render-failure retry (free):** a `target_status` of `613` or `5xx` is a transient render failure that arrives inside a *successful* Decodo HTTP 200 envelope (no `result.error`). Because Decodo does **not** bill these, `scrapeWithWait` retries them up to `RENDER_FAILURE_MAX_RETRIES` (2) times at no cost before giving up. This was added after a heavy MCO category page (`MCO175794`/`MCO180800`) was found to `613` under concurrent load yet render fully on the very next attempt — without the retry it was being silently dropped with 0 products.
- **Partial-render retry (billed):** a 200 with HTML <50 KB (head-only, the streaming-SSR race) is retried **once** (controlled by `SCRAPER_RETRY_PARTIAL_RENDER`, default on); if still partial, the page degrades to `EMPTY_ENRICHMENT`. Each retry is an extra billed request, which is why it is bounded to one.

Expected `4xx` (a genuine 404/410 = no /mas-vendidos page) is **never** retried — it is permanent, not transient.
...
**Important: a `613`/`5xx` is NOT "no page".** Only a genuine `4xx` (`>= 400 && < 500`, typically 404/410) means the page does not exist. A `target_status` of `613` ("Decodo failed to scrape") or a `5xx` is a *transient render failure* — the category may well have products that just failed to render this time (confirmed live: a category that `613`s under load renders 20 products on retry). The scraper therefore (a) retries `613`/`5xx` for free in `scrapeWithWait`, and (b) if it still fails, logs "render failed after retries" and returns 0 products **for that run only** — it does **not** flag the category as having no best-sellers. The `probeCategoryBestSellers` phase mirrors this: `613`/`5xx` → `failed` (category left untouched), never `no_page` (which would blacklist it). This distinction is what stops a transient Decodo glitch from permanently excluding a real category.
...
**Mitigation** (built into `MlScraperService`): every request chains `wait: 4s` → `scroll_to_bottom: 3s` → `wait_for_element: { selector, timeout_s: 15 }`, any response that still comes back as a partial render (<50 KB) is retried once (`SCRAPER_RETRY_PARTIAL_RENDER`), and any `613`/`5xx` render failure is retried up to 2× for free (`RENDER_FAILURE_MAX_RETRIES`). `wait_for_element` exits as soon as the selector appears, so the 15 s ceiling only lengthens slow/failing renders — healthy pages are unaffected and the $ cost per request is unchanged. The chain was validated at 100 % success across two verticals (`MLC1512` tools, `MLC1648` electronics, 42 pages each in standalone test); the timeout was later raised from 8 s to 15 s and the single retry added after a live run showed ML occasionally needs longer than 8 s to paint the price block. Without the chain, plain `wait_for_element` alone was insufficient — it failed identically when ML never rendered the selector, with the same head-only body.
...
### `scripts/test-decodo-async-ab.js` — SYNC vs ASYNC A/B

Standalone (no DB) comparison of Decodo's synchronous `/v2/scrape` against the asynchronous `/v3/task` flow (submit → poll status → fetch results), replaying a list of URLs through both modes at production-like concurrency and printing a side-by-side table (success rate, 400 rate, latency, billable count). Built to answer "does async mode prevent the transient MCO 400 bursts?" — the verdict (2026-06-13) was **no**: same 87.5 % success, ~2× slower (median 55 s vs 30 s), and the same category failed in both modes. The real cause turned out to be the `613` misclassification fixed in `MlScraperService`, not the request mode. Async **does** honor `proxy_pool: premium` + `browser_actions` (confirmed in the dumped task envelope) — it is simply slower with no reliability gain, so we stay on `/v2/scrape`.

Run:
```bash
node scripts/test-decodo-async-ab.js
# Key env: AB_SITE=MCO  AB_MODES=both|sync|async  AB_SAMPLE=12  AB_ROUNDS=1
#          AB_CONCURRENCY=10  AB_URL_FILE=mco-failed-urls.txt
#          AB_ASYNC_TIMEOUT_MS=200000 (async tasks can take >60 s each)
```
Raw first-response shapes are dumped to `tmp/decodo-ab/` for inspection. Keep as a smoke test to re-evaluate async if Decodo's pricing/latency for `/v3/task` changes.
