...
It can also be run from GitHub Actions via the **Bright Data Test** workflow (`.github/workflows/brightdata-test.yml`, `workflow_dispatch`). Use as a smoke test after changing Web Unlocker parameters or after Bright Data / ML changes that might affect rendering. ~$0.03 per run. When a request returns a 0-byte body, re-run that URL with `format: "json"` to read `status_code` / `x-brd-err-code`.
